...
namespace Polarity
{
	...
	Ref<Texture2D> TextureImporter::LoadTexture2D(const std::filesystem::path& path)
	{
		...
		{
			POLARITY_PROFILE_SCOPE("stbi_load: TextureImporter::ImportTexture2D");
		...
		{
			POL_CORE_ERROR("TextureImporter::ImportTexture2D: Could not load texture from filepath: {}", path.string());
			return nullptr;
		}
