...
namespace Polarity
{
	...
	void EditorLayer::OnAttach()
	{
		...
		// --------------------------------------------------- Panels
		m_EditorContext.Panels.emplace_back(CreateScope<AssetsPanel>());

		auto consolePanel = CreateScope<ConsolePanel>();

		AddLogListener([panel = consolePanel.get()](const LogEvent& e)
		{
			panel->Bind_Log(e);
		});
		m_EditorContext.Panels.emplace_back(std::move(consolePanel));

		m_EditorContext.Panels.emplace_back(CreateScope<HierarcyPanel> ());

		m_EditorContext.Panels.emplace_back(CreateScope<InspectorPanel>());

		auto menubar = CreateScope<MenubarPanel>();
		m_MenubarPanel = menubar.get();        // raw pointer
		m_EditorContext.Panels.emplace_back(std::move(menubar));

		auto viewport = CreateScope<ViewportPanel>();
		m_ViewportPanel = viewport.get();        // raw pointer
		m_EditorContext.Panels.emplace_back(std::move(viewport));
